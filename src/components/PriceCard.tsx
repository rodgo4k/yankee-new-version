...
const PriceCard = ({ name, price, period = "/ mês", tagline, features, cta, href = "#", highlighted, delay = 0 }: PriceCardProps) => (<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay }} className={`rounded-[2rem] border p-8 flex flex-col ${highlighted
    ...
    <a href={href} className={`mt-6 inline-flex items-center justify-center px-5 py-2.5 rounded-full text-[13px] font-medium transition-all duration-200 ${highlighted
